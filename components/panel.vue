<template>
  <div class="panelify-wrapper">
    <div class="panelify-controller">
      <div
        class="panelify-controller-button"
        v-for="(col, i) in themeBlocks"
        :key="i"
        @click="changeActiveBlock(col)"
        :style="getControllerButtonStyle(i, col)"
      ></div>
    </div>
    <div class="block-panel-wrapper" :style="getWrapperStyle()">
      <div class="block-panel-header" :style="getHeaderStyle()">
        ...
        <div v-if="!isBlock" style="padding-right: 10px;" class="block-panel-menu">
          <hamburger :color="getMenuColor()" />
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    activeController() {
      return this.themeBlocks.find(controller => {
        return controller.active;
      });
    }
  },
  watch: {
    activeController(item) {
      this.updatePanelifyTheme();
    }
  },
  data: () => ({
    newTheme: null,
  ...
  methods: {
    ...
    getTabStyle() {
      return `
        border-width: ${this.isBlock ? "0px 0px 2px 0px" : "0px"};
        border-color: ${starlette.getColorAs(
          "color-btn-pill-border",
          this.appName,
          this.newTheme || this.theme,
